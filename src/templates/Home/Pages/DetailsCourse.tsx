...
export default function DetailsCourse(props: any) {
  const { courseDetail } = useSelector((state: any) => state.CoursesReducer);

  const { arrCourses } = useSelector((state: any) => state.CoursesReducer);
  ...
  const renderSimilarCourse = () => {
    ...
    return similarCourses
      ?.filter((course: any) => course.tenKhoaHoc !== courseDetail.tenKhoaHoc)
      .slice(0, 8)
